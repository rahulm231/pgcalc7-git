...
;(function($, win, doc) {
	"use strict";
	$.iModAnythingSlider = function(el, options) {

		var base = this, o, t;

		// Wraps the ul in the necessary divs and then gives Access to jQuery element
		base.el = el;
		base.$el = $(el).addClass('anythingBase').wrap('<div class="iModAnythingSlider"><div class="anythingWindow" /></div>');

		// Add a reverse reference to the DOM object
		base.$el.data("iModAnythingSlider", base);

		base.init = function(){

			// Added "o" to be used in the code instead of "base.options" which doesn't get modifed by the compiler - reduces size by ~1k
			base.options = o = $.extend({}, $.iModAnythingSlider.defaults, options);
			...
			// base.$el = original ul
			// for wrap - get parent() then closest in case the ul has "iModAnythingSlider" class
			base.$wrapper = base.$el.parent().closest('div.iModAnythingSlider').addClass('iModAnythingSlider-' + o.theme);
		...
		base.updateSlider = function(){
			...
			base.$items.find('a').unbind('focus.iModAnythingSlider').bind('focus.iModAnythingSlider', function(e){
		...
		base.endAnimation = function(page, callback, time){
			...
			// callback from external slide control: $('#slider').iModAnythingSlider(4, function(slider){ })
...
*/
	...
	$.fn.iModAnythingSlider = function(options, callback) {

		return this.each(function(){
			var page, anySlide = $(this).data('iModAnythingSlider');

			// initialize the slider but prevent multiple initializations
			if ((typeof(options)).match('object|undefined')){
				if (!anySlide) {
					(new $.iModAnythingSlider(this, options));
				} else {
					anySlide.updateSlider();
				}
			// If options is a number, process as an external link to page #: $(element).iModAnythingSlider(#)
