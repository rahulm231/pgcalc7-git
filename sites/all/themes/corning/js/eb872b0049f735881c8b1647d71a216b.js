(function ($) {
  ...
  var cloudzoomSettings = function (cx, s) {
    $img = $('img.cloudzoom', cx);

    $img.each(function (i, el) {
      var rel = $(el).data('cloudzoom');
      var style = $(el).attr('style');
      var src = el.src.split('/');
      var start = 6;
      var length = 3;
      var base = Drupal.settings.basePath.split('/');
      for (i in base) {
        if (!!base[i].length) {
          start++;
        }
      }
      src.splice(start, length);
      src = src.join('/');

      $(el).wrap('<a class="cloud-zoom" href="' + src + '" rel="' + rel + '" style="' + style + '"></a>');
    });


    if ($.isFunction($.fn.on)) {
      // For jQuery 1.7+
      $('.image-caption-container').on('click', '.image-caption', function (e) {
        e.preventDefault();
      ...
      $('.image-caption').live('click', function (e) {
        e.preventDefault();
        e.stopImmediatePropagation();
        return false;
      });
  ...
  Drupal.behaviors.image_caption = {
    attach: function (context, settings) {
      $("img.caption:not(.caption-processed)").each(function(i) {
        ...
        // Display inline block so it doesn't break any text aligns on
        // the parent contatiner.
        $(this).wrap("<span class=\"image-caption-container\" style=\"display:inline-block;" + style + "\"></span>");
        // Customization to add class.
        $(this).parent().addClass($(this).attr('class'));
        $(this).parent().addClass('image-caption-container-' + alignment);
        // Customization to add class.
        $(this).addClass('alpha omega');

        // Removed here: Add dimensions, if available (from image_caption module)
        // Append caption.
...
(function($) {
    ...
    Drupal.behaviors.cmogBanner = {
        attach: function(cx, s) {

            var win = $(window),
                rotator = $('.view-banner-rotator .view-content'),
                images = $('img', rotator);

            // Because the elements are initially hidden, x-browser inconsistencies prohibit grabbing
            // the height and width attribute accurately.  By matching the html, we can rely on a
            // consistent result.
            var width = parseInt(images[0].outerHTML.match(/width=["]?([\d]+)["]?/i)[1]),
                height = parseInt(images[0].outerHTML.match(/height=["]?([\d]+)["]?/i)[1]),
                ratio = height / width;

            var setWidth = function() {
                var windowWidth = win.width(),
                    rimgHeight  = ratio * windowWidth,
                    minWidthValue = '',
                    marginLeftValue = '',
                    marginTopValue = '';

                if (windowWidth >= width) {
                    minWidthValue = windowWidth;
                    marginLeftValue = -(windowWidth / 2);
                    marginTopValue = -(rimgHeight / 2);
                }
...
;
(function($){
  var commerceCorningAddToCartInit = function(cx, s) {
    $('#cart-link').once('addToCartTrigger', function(){
      if (!!s.commerceCorning && !!s.commerceCorning.toggle_button) {
        $('#toggle_button_wrapper input').click(function(e){
          e.preventDefault();
          $(e.target).closest('form').toggleClass('toggle-button');
          return false;
        });
      }

      $('#cart-link').change(function(e){
        ...
        $.colorbox({
            ...
            fixed: true,
            opacity: 0.6,
            html: $('#cmog_ajax_cart').clone().attr('id', 'cmog_ajax_cart_modal'),
            onOpen: function(){
              $('#colorbox').addClass('login-colorbox');
            },
            onClosed: function(){
              $('#colorbox').removeClass('login-colorbox');
            }
        });
      });

      $('#main-content').bind('error', function(e){
        $('html, body').animate({ scrollTop: $('.messages').eq(0).offset().top + 40 }, 300);
      });
    });
  }

  Drupal.behaviors.commerceCorningAddToCart = {
    attach: commerceCorningAddToCartInit
  }
