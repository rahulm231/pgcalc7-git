...
jQuery(function(jQuery) {
    ...
    var jQuerynavMain = jQuery('.nav-main');
    var jQuerynavSub = jQuery('.nav-sub');
    var jQuerynavMain_level1 = jQuery('#fsHeader').find('.nav-main .fsNavLevel1');
    var sectionTitle = jQuerynavMain_level1.find('> li[class*="fsNavCurrentPage"] > a').text();
    var jQuerynavSub_title = jQuerynavSub.find('> header > .fsElementTitle');
    var bpMobile = 600;
    var bpTablet = 800;
    var isHome = jQuery('.home').length;
    var notDraftMode = !jQuery('.fsDraftMode').length; // if (isHome && notDraftMode)....
    ...
    jQuery('blockquote').wrapInner('<div class="block-wrapper"></div');

    // ================================
    // 5050 Slideshow Treatment
    // ================================
    /*
    jQuery(".fsAthleticsEvent.custom-scroll .fsListItems").slick({
      mobileFirst:true,
      responsive: [
        {
          breakpoint: 650,
    ...
    // 5050 Slideshow Treatment
    // ================================
    if(jQuery(".fiftyfiftySlider").length) {
      var jQuerybpaSlider = jQuery(".fiftyfiftySlider");
      jQuerybpaSlider.each(function() {
    ...
    // Main Search
    // ================================
    jQuery('.header-top .search-trigger-container').click( function() {
      jQuery('.top-search, .search-trigger-container, .nav-utility-header').addClass('active');
    });

    jQuery(document).on('click', function(event) {
      if (!jQuery(event.target).closest('.search-trigger-container, #fsHeader .top-search').length) {
        jQuery('.top-search, .search-trigger-container, .nav-utility-header').removeClass('active');
      }
    });
    ...
    jQuery(function(jQuery) {
      var num_cols = 2,
      container = jQuery('.nav-main .fsNavLevel2'),
      listItem = 'li',
    ...
    jQuerynavMain_level1.find('.fsNavPageThumbnail > img').wrap("<div class='thumbnail-wrapper'></div");
    jQuerynavMain_level1.find('.fsNavPageDescription > img').wrap("<p class='fs_style_31'></p>");

//    jQuery('.nav-main .fsNavLevel1 > li').doubleTapToGo();

//    jQuery('.nav-utility-header .fsNavLevel1 > .fsNavParentPage').doubleTapToGo();
    ...
    if((!jQuery(".home").length) && (!jQuery(".interior-hero").length) && (!jQuery(".landing-slider").length)) {
      jQuerybody.addClass('no-hero');
      fixedHeaderNoHero();
    }

    var jQuerymenuStick = jQuery('.nav-main').offset().top;

    jQuerynavMain_level1.prepend('<a href="/" class="fixed-logo"></a>');

    function fixedHeader() {
      if (jQuery(window).width() > 750) {
        if (jQuery(window).scrollTop() >= jQuerymenuStick) {
          jQuerynavMain.addClass('is-fixed');
          jQuery('.search-trigger-container').appendTo('.is-fixed > .fsElementContent > nav > .fsNavLevel1');
          jQuery('.top-search').appendTo('.is-fixed > .fsElementContent > nav > .fsNavLevel1');
          if (jQuery('#fsAccountBar').length) {
            jQuerynavMain.addClass('fixed-account');
          } else {
            jQuerynavMain.removeClass('fixed-account');
          }
        } else {
          jQuerynavMain.removeClass('is-fixed');
          jQuery('.search-trigger-container').insertAfter('.header-top .seo-content');
          jQuery('.top-search').insertBefore('.header-top .nav-utility-header');
        }
      }
    }
    ...
    function fixedHeaderNoHero() {
      ...
      if (jQuery(window).width() > 750) {
        if (jQuery(window).scrollTop() >= 1) {
          ...
          jQuery('.search-trigger-container').appendTo('#fsHeader .nav-main .fsNavLevel1');
          jQuery('.top-search').appendTo('#fsHeader .nav-main .fsNavLevel1');
          if (jQuery('#fsAccountBar').length) {
            jQuerynavMain.addClass('fixed-account');
          } else {
            jQuerynavMain.removeClass('fixed-account');
    ...
    if(jQuery('body:not(.fsComposeMode)').length){
      var scroll = false;
    ...
    if(jQuery('body:not(.fsComposeMode)').length) {
      jQuery('.interior-hero').appendTo('#fsHeader > .fsBanner');
      jQuery('.landing-slider').appendTo('#fsHeader > .fsBanner');
      jQuery('.fsAthleticsEvent.custom-scroll').appendTo('#fsHeader > .fsBanner');
    }
    jQuery('.landing-slider').each(function() {
    ...
    HOME = {
        ...
        slideshow: function() {
          ...
          jQuery('.video-slide').each(function() {
            ...
            var jQueryheroVideoContainer = jQuery(this).find('.fsMediaCustomPlayer');
            var videoURL = jQueryheroVideoContainer.attr('data-playlisturl');

            jQuery.getJSON(videoURL, function(data) {

                jQuery.each(data.objects, function(i, object) {
                    // jQuery('<div class="vid-inner"  ></div>');
                    jQuery('<div class="vid-inner" style="background-image: url('+object.display_path+');"></div>')
                    ...
                    .prependTo(jQueryheroVideoContainer);
                });
            }).done(function() {


            var mediaContainer = jQuery('.hero-media-container .fsElementContent'),
                tabContainer = jQuery('.hero-tabs-container  .fsElementContent'),
                // mediaArray = jQuery('.hero-media-container > .fsElementContent > .hero-slide'),
                // tabsArray = jQuery('.hero-tabs-container  .fsElementContent > .hero-tab');
                mediaArray = jQuery('.hero-slide'),
                tabsArray = jQuery('.hero-tab');

            jQuery('video').attr('id', 'hero-vid');
            jQuery('.hero-slide:last-child').addClass('last');
            jQuery('.hero-slide:first-child').addClass('first active-slide');
            jQuery('.hero-tab:last-child').addClass('last');
            jQuery('.hero-tab:first-child').addClass('first active-tab');
            jQuery('.hero-slide').addClass('start-slide');
            jQuery('.hero-tab').addClass('start-tab');

            var jQueryactiveSlide = jQuery('.active-slide');
            var video = jQueryactiveSlide.find('#hero-vid');
            var notActiveVideo = jQuery('.hero-slide:not(.active-slide)').find('#hero-vid');

            var jQueryheroSlide = jQuery('.hero-slide');
            var jQueryheroTab = jQuery('.hero-tab');
            ...
            if (jQuery(window).width() > 699) {
              ...
              jQueryheroSlide.find('video').on('ended', function(event){
                var thisSlide = jQuery(this).parents('.start-slide');
                var nextSlide = thisSlide.next();
                var nextVideo = nextSlide.find('#hero-vid');
                var tabIndex = nextSlide.index();
                var jQueryfirstSlide = jQuery('.start-slide.first');
                var jQueryfirstTab = jQuery('.hero-tab.first');
                ...
                if (thisSlide.hasClass('last')) {
                  tabsArray.removeClass('active-tab');
                  jQueryfirstSlide.find('#hero-vid').get(0).play();
                  jQueryfirstSlide.addClass('active-slide');
                  jQueryfirstTab.addClass('active-tab');
                } else if (thisSlide.hasClass('start-slide')){
                  tabsArray.removeClass('active-tab');
                    jQuery('.hero-media-container > header').addClass('overlay-fade');
                    nextVideo[0].play();
                    jQueryheroSlide.find('video').currentTime = 0;
              ...
              // jQuery('.hero-media-container').videoSlider();

              jQuery('.hero-tab').each(function() {
                var _ = jQuery(this);
                var slideIndex = _.index();

                _.on({
                  mouseenter: function(e){
                    jQueryheroSlide.removeClass('start-slide');
                    jQueryheroTab.removeClass('start-tab');

                    jQuery('.hero-slide:not(.active-slide)').find('#hero-vid')[0].pause();
                    jQuery('.hero-slide').find('#hero-vid').removeAttr('loop', 'loop');

                    // jQuery('#hero-vid').get(0).pause();

                    jQueryheroTab.removeClass('active-tab');
                    jQueryheroSlide.removeClass('active-slide');
                    ...
                    // jQueryheroSlide.find('video').get(0).pause();
                    jQueryheroSlide.eq(tabIndex).addClass('active-slide');
                    jQueryheroSlide.eq(tabIndex).find('#hero-vid').get(0).currentTime = 0;
                    jQueryheroSlide.eq(tabIndex).find('#hero-vid')[0].play();
                    jQueryheroSlide.eq(tabIndex).find('#hero-vid').attr('loop','loop');
                  },

                  mouseleave:function(e){
                    // jQuery('#hero-vid').get(0).pause();
                    var tabIndexOut = jQuery(this).index();
                    if (! jQuery(e.toElement).hasClass('hero-tab')) {

                      jQueryheroTab.removeClass('hovered');


                      // jQuery('.active-slide').find('#hero-vid').on('ended', function(){
                      jQuery('.hero-slide').eq(tabIndexOut).find('#hero-vid').on('ended', function(){
                        var jQueryactiveSlideOut = jQuery(this).parents('.hero-slide');
                        var slideIndexOut = jQueryactiveSlideOut.index();

                        jQuery(this).attr('loop', 'loop');

                        // jQueryactiveSlideOut.removeClass('active-slide');

                        // jQuery('.hero-slide').find('#hero-vid').get(0).currentTime = 0;
                        // jQuery(this).get(0).play();
                        // jQueryheroTab.eq(slideIndexOut).addClass('active-tab');

                        // if (jQueryactiveSlideOut.hasClass('last')) {
                        //   jQuery('.hero-slide.first').find('#hero-vid').get(0).play();
                        //   jQuery('.hero-slide.first').addClass('active-slide');
                        //   jQuery('.hero-tab.first').addClass('active-tab');
                        //   jQuery('.hero-slide.first').find('#hero-vid').attr('loop', 'loop');
                        // } else {
                        //   jQueryactiveSlideOut.next().addClass('active-slide');
                        //   jQueryheroTab.eq(slideIndexOut).next().addClass('active-tab');
                        //   jQueryactiveSlideOut.next().find('#hero-vid').get(0).currentTime = 0;
                        //   jQueryactiveSlideOut.next().find('#hero-vid')[0].play();
                        //   jQueryactiveSlideOut.next().find('#hero-vid').attr('loop', 'loop');
                        // }
                      ...
                      // jQueryheroSlide.addClass('restart-slide');

                      // jQuery('restart-slide').eq(tabIndexOut).find('#hero-vid').on('ended', function(){
                      //   var newVideo = jQuery(this).parents('.hero-video');
            ...
            // }
                ...
                jQuery('.hero-tab').hover(function(){
                  if (jQuery(this).hasClass('active-tab-mobile')) {
                    return true;
                  } else {
                    jQuery('.hero-tab').removeClass('active-tab-mobile');
                    jQuery('.hero-slide').removeClass('active-slide');
                    jQuery(this).toggleClass('active-tab-mobile');
                ...
                jQueryheroVideoContainer.append('<span>Please make sure you have content added to media manager and that you have selected the correct element settings.</span>').css('textAlign', 'center');
    ...
    jQuery(window).scroll(function() {
      ...
      if (jQuery('.voices-container').length){
        var voicesTop = jQuery('.voices-container').offset().top;
        if ((voicesTop-300) < scrollTop) {
          jQuery('.voices-container > header').addClass('animate-line');
        } else {
          jQuery('.voices-container > header').removeClass('animate-line');
        }
      }
      if (jQuery('.infographic-container').length){
        var infoTop = jQuery('.infographic-container').offset().top;
        if ((infoTop-300) < scrollTop) {
          jQuery('.infographic-container > header').addClass('animate-line');
        } else {
          jQuery('.infographic-container > header').removeClass('animate-line');
        }
      }
      if (jQuery('.news-container').length){
        var newsTop = jQuery('.news-container').offset().top;
        if ((newsTop-300) < scrollTop) {
          jQuery('.news-container > header').addClass('animate-line');
        } else {
          jQuery('.news-container > header').removeClass('animate-line');
        }
      }
      if (jQuery('.video-header').length){
        var videoTop = jQuery('.video-header').offset().top;
        if ((videoTop-300) < scrollTop) {
          jQuery('.video-header > header').addClass('animate-line');
        } else {
          jQuery('.video-header > header').removeClass('animate-line');
        }
      }
    });
    // } else {
    //   jQuery(window).scroll(function() {
    //     var scrollTop = jQuery(window).scrollTop(),
    //       infoTop = jQuery('.infographic-container').offset().top;
    ...
    if (notDraftMode) {
      jQuery('.home:not(.fsComposeMode) .voices-container > .fsElementContent').slick({
        slidesToShow:  1,
        slidesToScroll: 1,
        arrows: false,
        dots: true,
    ...
    jQuery('.voices-slide').each(function(){
      jQuery(this).find('.profile--style5').addClass('in-slide');
    });
    ...
    // ================================
    setTimeout(function() {
      if(jQuery("body:not(.home) .infographic-container > .fsElementContent").width() !== jQuery("body:not(.home) #fsPageContent").width()) {
        jQuery("body:not(.home) .infographic-container").addClass("along-left-banner");
      }
      if(jQuery("body:not(.home) .voices-container > .fsElementContent").width() !== jQuery("body:not(.home) #fsPageContent").width()) {
        jQuery("body:not(.home) .voices-container").addClass("along-left-banner");
      }
    },1000);
    jQuery(window).on("resize",function() {
      if(jQuery("body:not(.home) .infographic-container > .fsElementContent").width() == jQuery("body:not(.home) #fsPageContent").width()) {
        jQuery("body:not(.home) .infographic-container").removeClass("along-left-banner");
      } else {
        jQuery("body:not(.home) .infographic-container").addClass("along-left-banner");
      }
      if(jQuery("body:not(.home) .voices-container > .fsElementContent").width() == jQuery("body:not(.home) #fsPageContent").width()) {
        jQuery("body:not(.home) .voices-container").removeClass("along-left-banner");
      } else {
        jQuery("body:not(.home) .voices-container").addClass("along-left-banner");
      }
    });

    var infoSlides = jQuery('.infographic-container > footer > .fsElementFooterContent').text();

    jQuery('body:not(.fsComposeMode) .infographic-container > .fsElementContent').slick({
    ...
    jQuery.fn.setIcon = function() {
      ...
      jQuery('.infographic-stat .fsElementHeaderContent').each(function(i) {
        var iconGet = jQuery(this).text();
        jQuery(this).append("<i></i>");
        jQuery(this).find('i').addClass(iconGet);
      });
    };

    jQuery('.home:not(.fsComposeMode) .info-box').setIcon();

    // ================================
    // Visitation Voices
    // ================================

    jQuery('.profile .fsElementFooterContent').wrapInner('<div class="footer-wrap"></div>');

    jQuery('.profile .fsElementFooterContent').each(function(){
      jQuery(this).find('a').appendTo(this);
    });
	/*
    jQuery('.profile:not(.profile--style5) .footer-wrap').dotdotdot({
      watch: window
    });
	
	
    jQuery('.profile--style5 .footer-wrap em').dotdotdot({
      watch: window
    });
	*/
	
    jQuery('.profile.profile--style5 img').wrap('<div class="border-wrapper"></div>');

    // ================================
    // Image Callouts
    // ================================
    jQuery(".image-callout").each(function() {
      var anchor = jQuery(this).find(">.fsElementContent a").first();
      anchor.clone().addClass("overlay").prependTo(jQuery(this));
    });
    ...
    jQuery('.news-home article').each(function() {
      jQuery(this).children().not('.fsThumbnail').wrapAll('<div class="news-content"></div>');
      if (jQuery(this).find('.fsThumbnail').length) {
        return true;
      } else {
        jQuery(this).addClass('no-image');
        jQuery(this).prepend("<a class='fsThumbnail fsNewsPostLink' href='#'><img src='/uploaded/themes/default_16/images/news-fallback.png' alt='News Default Image'></a>");
      }
    });

    jQuery('.calendar-home article').unwrap();
    jQuery('.calendar-home').each(function(){
      var dates = jQuery('article', this);
      for(var i = 0; i < dates.length; i+=3) {
        dates.slice(i, i+3).wrapAll('<div class="slide-wrapper"></div');
      }
    });

    jQuery('.calendar-home > .fsElementContent > .fsListItems').slick({
      slidesToShow:  1,
      slidesToScroll: 1,
      arrows: false,
      dots: true,
    ...
    // jQuery('.fsAthleticsEvent.fsList article').each(function(){
    //   var jQueryeventTime = jQuery(this).find('.fsDateTime');
    //   jQuery(':not(.fsDate)', jQueryeventTime).children().wrapAll('<div class="time-wrapper"></div>');
    //   jQuery('time-wrapper').insertBefore(jQuery(this).find('.fsTitle'));
    // });
    ...
    if(jQuerynavSub.find('nav .fsNavLevel1').length !== 0) {
      jQuerynavSub.removeClass( 'nav-sub-empty' );
    } else {
      jQuerynavSub.addClass( 'nav-sub-empty' );
    }

    // nav-sub - mobile toggle
    jQuerynavSub_title.click( function() {
      jQuery(this).closest( '.nav-sub' ).toggleClass( 'active-nav' );
    });

    jQuery(document).on('click', function(event) {
      if ( !jQuery(event.target).closest( '.nav-sub' ).length ) {
        jQuerynavSub.removeClass( 'active-nav' );
      }
    });

    jQuery('.header-top > .fsElementContent').prepend([
      '<div class="drawer-ribbon">',
      '<button class="drawer-trigger" href="#"></button>',
      '</div>'
    ].join('\n'));

    jQuery('.fsMenu').prepend('<div class="close-btn"></div>');

    jQuery('.drawer-trigger').click(function() {
      jQuerybody.toggleClass('drawer-is-active');
    });

    jQuery('.close-btn').click(function(){
      jQuery('body').removeClass('drawer-is-active');
    });

    jQuery('.mobile-nav li.fsNavParentPage').prepend('<div class="drop-trigger"></div>');

    jQuery('.drop-trigger').click(function() {
      jQuery(this).toggleClass('expanded');
      jQuery(this).parent().find('> .fsNavPageInfo').slideToggle();
      jQuery(this).parent().toggleClass('active');
      //need to make sure that current pages get the active class added to them by default and dont get it stuck to them
    });

    jQuery('.fsMenu .mobile-nav ul li.fsNavCurrentPage').addClass('active');
    jQuery('.fsMenu .mobile-nav ul li.fsNavCurrentPageAncestor').addClass('active');

    jQuery('.fsMenu .mobile-nav ul li.fsNavCurrentPage > .drop-trigger').addClass('expanded');
    jQuery('.fsMenu .mobile-nav ul li.fsNavCurrentPageAncestor > .drop-trigger').addClass('expanded');

    jQuery('.utility-news-calendar').clone().appendTo('.drawer-ribbon');
    jQuery('.utility-header .search-trigger-container').clone().appendTo('.drawer-ribbon');

    jQuery('#fsMenu .nav-utility-header .fsNavLevel1 > li:first-child > a').text('Login');

    jQuery('.drawer-ribbon .search-trigger-container').click(function() {
      jQuerybody.toggleClass('drawer-is-active');
    });
    ...
    jQuery('.nav-sub').prepend('<div class="section-title"><a href="/'+sectionTitle+'">' + sectionTitle + '</a></div>');
    jQuery('.section-title:empty').remove();
    ...
    jQuery('.section-title a').click(function(e) {
    ...
    jQuery('.fsBreadcrumb .fsNavBreadcrumbSeperator').text("\u2014");

    if(jQuery('body:not(.style-guide):not(.fsComposeMode)').length) {
      jQuery('.fsBreadcrumb').prependTo('#fsPageBody');
    }
    ...
    UTIL = {
        ...
        respondSliders: function() {
            ...
            for (var i = 0; i < targets.length; i++) {
                jQuery(targets[i] + ' .fsElementSlideshow')
        ...
        announcement: function() {
          ...
          if (jQuery('.announcement').length) {
            jQuery('.announcement').before('<div class="announcement-tag">Announcement Open</div>');
            jQuery('.announcement header').before('<div class="close-btn"></div>');
          }

          if(notDraftMode) {
          jQuery(window).on('load',function(){
            jQuery('.announcement').insertAfter('#fsHeader > .fsBanner');
            jQuery('.announcement-tag').insertAfter('#fsHeader > .fsBanner');
          });
          }

          jQuery('.announcement .close-btn').on('click', function() {
            jQuery('.announcement, .announcement-tag').addClass('hidden');
          });
          jQuery('.announcement-tag').on('click', function() {
            jQuery('.announcement, .announcement-tag').removeClass('hidden');
          });
    ...
    jQuery('.module').each(function() {
      jQuery(this).find('img').prependTo(jQuery(this)).wrap('<div class="image-wrapper"></div>');
    });
