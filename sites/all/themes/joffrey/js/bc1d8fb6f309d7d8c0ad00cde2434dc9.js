...
;
Drupal.behaviors.cc_viewshomepagelist = function(context) {

  //load images with JS
  if (Drupal.settings.cc_viewshomepagelist) {  
	  if (Drupal.settings.cc_viewshomepagelist.images_to_load) {
	    
	    for(key in Drupal.settings.cc_viewshomepagelist.images_to_load) {
	      cc_viewshomepagelist_load_image(Drupal.settings.cc_viewshomepagelist.images_to_load[key]);
	    }    
	  }
	}  

  $(".viewshomepagelist h2 a").click(function () {
	  var parent_li = $(this).parent().parent();
	  var li_id = parent_li.attr('id');
	
	  $('.viewshomepagelist').removeClass('pseudo-active');
	  $(parent_li).addClass('pseudo-active');  
	  
	  //Hide all rows
	  $('.view-display-id-panel_pane_homepagemain .views-row.mainbody-homepage-row').hide();
	  //then show the one we want to show
	  $('.mainbody-' +  li_id).show();
  ...
  $(".play-video-link").click(function () {
	  //alert('hello');
		...
		$(this).parent().parent().find('.main-image').hide() ;
		$(this).parent().parent().find('.main-video').show() ;
		
		
	  //switch the play and stop links
	  $(this).hide();
	  $(this).parent().parent().find('.stop-video-link').show();
	  
    return false;
  });


  $(".stop-video-link").click(function () {
	  //alert('hello');
		
		$(this).parent().parent().find('.main-video').hide() ;
		$(this).parent().parent().find('.main-image').show() ;
		
		
	  //switch the play and stop links
	  $(this).hide();
	  $(this).parent().parent().find('.play-video-link').show();
 ...
 * Preload an image
 */
function cc_viewshomepagelist_load_image(path) {
  heavyImage = new Image(); 
  heavyImage.src = path;
}


function cc_viewshomepagelist_play_vid(li_id) {


  //switch the image and the video
  $('.mainbody-' + li_id + ' .main-image').hide();
  $('.mainbody-' + li_id + ' .main-video').show();

  //switch the play and stop links
  $('.mainbody-' + li_id + ' .play-video-link').hide();
  $('.mainbody-' + li_id + ' .stop-video-link').show();

}


function cc_viewshomepagelist_stop_vid(li_id) {


  //switch the image and the video
  $('.mainbody-' + li_id + ' .main-video').hide();
  $('.mainbody-' + li_id + ' .main-image').show();


  //switch the play and stop links
  $('.mainbody-' + li_id + ' .stop-video-link').hide();
  $('.mainbody-' + li_id + ' .play-video-link').show();
