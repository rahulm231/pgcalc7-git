...
function wbk_renderSetDate( scroll ) {
    ...
    jQuery.post( wbkl10n.ajaxurl, data, function(response) {
		...
		if( wbkl10n.date_input == 'popup' || wbkl10n.date_input == 'classic'  ){
			...
			if ( wbkl10n.mode == 'extended' ){
				jQuery('#wbk-date-container').html( sep_html + '<div class="wbk-col-12-12"><label class="wbk-input-label">' + wbkl10n.selectdatestart + '</label><input value="' + wbkl10n.selectdate + '" type="text" class="wbk-input"  ' + prefil_date + ' id="wbk-date" /></div>');
			} else {
				jQuery('#wbk-date-container').html(  sep_html + '<div class="wbk-col-12-12"><label class="wbk-input-label">' + wbkl10n.selectdatestartbasic + '</label><input value="' + wbkl10n.selectdate + '" type="text" ' + prefil_date + ' class="wbk-input" id="wbk-date" /></div>');
		 	}
		 	if( prefil_date != ''){
			 	if ( wbkl10n.mode == 'extended' ){
	 				wbk_clearSetTime();
...
function wbk_searchTime() {
    ...
    jQuery.post( wbkl10n.ajaxurl, data, function(response) {
    	...
    	if ( response == 0 || response == -1 || response == -2 || response == -3 || response == -4 || response == -4 || response == -6 || response == -7){
    		...
    		jQuery(wbkl10n.scroll_container).animate({ scrollTop: jQuery('#wbk-slots-container').offset().top - 120 }, 1000);
    	...
    	if (response == 0 || response == -1){
    		...
    		response_obj = jQuery.parseJSON( response );
     		if( response_obj.dest == 'slot' ){
      		...
      		jQuery(wbkl10n.scroll_container).animate({ scrollTop: jQuery('.wbk-more-container').last().offset().top - 120 }, 1000);
		...
		if( wbkl10n.multi_booking == 'enabled' || wbkl10n.multi_booking == 'enabled_slot' ){
			...
			if( selected_cnt > 0 ){
				...
				jQuery('#wbk-to-checkout').click(function() {
					...
					jQuery(wbkl10n.scroll_container).animate({ scrollTop: jQuery('#wbk-booking-form-container').last().offset().top - 120 }, 1000);
...
function wbk_set_payment_events(){
	jQuery('.wbk-payment-init').click(function() {
		...
		jQuery(wbkl10n.scroll_container).animate({ scrollTop: jQuery('#wbk-payment').last().offset().top - 120 }, 1000);
