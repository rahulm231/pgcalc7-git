(function($){
  js = {
   /* init: function() {
	...
	resizing: function(){
	    if($('body').has('#psu-emergency-alert')){
		    var aHi = $('#psu-emergency-alert').outerHeight();
		    $('body').css({paddingTop:aHi});
		}
		var ww = $(window).width();
...
jQuery(document).ready(function() { 
	...
	$(window).load(js.resizing);
	    ...
	    $('.gsc-search-box .gsc-input *').click(function(e){
		   e.preventDefault(); 
	    });
	    ...
	    $('.nav-opener').live('click',function(e){
		    e.preventDefault();
			var parent = $(this).parent();
			if(parent.hasClass('nav-active')){
				$('.nav-slide .mobile-container').slideUp(200,function(){
			    	parent.removeClass('nav-active');
				});
		    } else {
				$('.nav-slide .mobile-container').slideDown(300,function(){
			    	parent.addClass('nav-active');
				});
		    }
		});

		$('.widget .opener, .menu-mobile .opener').live('click',function(){
			var parentt = $(this).parent().parent();
			if(parentt.hasClass('active')){
				parentt.find('>.slide').slideUp(200,function(){
					parentt.removeClass('active');
				});
				$(this).find('i').removeClass('fa-chevron-up').addClass('fa-chevron-down');
			} else {
				parentt.parent().parent().find('li > .slide').slideUp(200,function(){
					parentt.parent().parent().find('li').removeClass('active');
				});
				parentt.parent().parent().find('li > span > .opener i').removeClass('fa-chevron-up').addClass('fa-chevron-down');
				parentt.find('>.slide').slideDown(200,function(){
					parentt.addClass('active');
				});
				$(this).find('i').removeClass('fa-chevron-down').addClass('fa-chevron-up');
			}
		});
		$('#mb-mobile li:not(.active) .opener').live('click',function(){
			var that = $(this),
				parentt = that.parent().parent();
			if(parentt.hasClass('opened')){
				parentt.find('>.slide').slideUp(200,function(){
					parentt.removeClass('opened');
				});
				that.find('i').removeClass('fa-chevron-up').addClass('fa-chevron-down');
			} else {
				parentt.parent().parent().find('li > span > .opener i').removeClass('fa-chevron-up').addClass('fa-chevron-down');
				parentt.parent().parent().find('li > .slide').slideUp(200,function(){
					parentt.parent().parent().find('li').removeClass('opened');
				});
				parentt.find('>.slide').slideDown(200,function(){
					parentt.addClass('opened');
				});
				that.find('i').removeClass('fa-chevron-down').addClass('fa-chevron-up');
			}
		});
		
		$('#mb-mobile .active .opener, #mb-mobile .current-page-parent .opener').click(function(e){
			e.preventDefault();
			var tt = $('.mobile-container .menu-mobile h3').offset().top-46;
			$('html,body').animate({scrollTop: tt},'slow');
		});
		
	    $('.nav-slide > a').live('click',function(){
			$(this).parent().addClass('open');
	    });
	    
	    $('.exp-box h4').live('click',function(e){
			e.preventDefault();
			var that = $(this);
		...
		$('.fcbox').fancybox({
			...
			helpers : {
				...
				overlay: {
					css: {'background':'rgba(230,235,240,.9)'}
				}
			}
		});
		$('.fcbox-iframe').fancybox({
			padding : 0,
			width: '80%',
			height: '80%',
			type: 'iframe',
