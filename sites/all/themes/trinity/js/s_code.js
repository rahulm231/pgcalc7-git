...
/* for a different site - change the s_account with new Report Suite, as well as add site to s.linkInternalFilters */
var s_account="tsc-"+orgId+",tsc-externalidglobal";
var s=s_gi(s_account);


// SiteCatalyst Code Version
// HARD CODED: [Sitecatalyst code version + internal version number]
// PLEASE CHANGE THIS "INTERNAL VERSION NUMBER" WHEN CHANGES TO THE S_CODE ARE MADE
var SCCodeVersion="SC25.2.2"
...
/* Cocatenate site prefix to page name, Returns page name as lowercase */
	if(s.prop17 && s.prop17!=""){
		s.pageName=s.prop17 + "||" + s.pageName;
	}
...
/* External Campaigns: places campaigns in order thats specified below */
	s.campaign=s.getValOnce(s.getQueryParam('cid'),'ext_campaign',30);

// Internal Campaigns: 
	s.eVar2=s.getValOnce(s.getQueryParam('intcmp'),'int_campaign',30);
		
//Get previous page value
	s.prop8=s.getPreviousValue(s.pageName,'gpv_pn','event3');
	s.eVar7=s.prop8;	

// Page URL excluding Query Strings
	//s.prop11=location.protocol + "//" + location.host + location.pathname
...
/*Day and time parting eVars   */
/*Please change the second parameter in the time parting function to the time zone require with reference to GMT */
	var currentYear = new Date().getFullYear( );	
		
// getTimeParting without year check
	s.prop5=s.eVar3=s.getTimeParting('h','0'); // Set hour 
	s.prop6=s.eVar4=s.getTimeParting('d','0'); // Set day 
	s.prop7=s.eVar5=s.getTimeParting('w','0'); // Set weekday
	
/* Visit Number */
	s.eVar9=s.getVisitNum();
	if(s.eVar9 && s.eVar9!=""){
		s.prop13=s.eVar9;
	}
	
//HARD CODED: [Sitecatalyst code version + internal version number]
	// Set at top of this file
	s.prop14=SCCodeVersion;	
	
/* Days Since Last Visit */
	s.prop15=s.getDaysSinceLastVisit();
    s.eVar10=s.prop15;	
...
s.visitorNamespace="thesteltercompany"
s.trackingServer="thesteltercompany.d1.sc.omtrdc.net"
